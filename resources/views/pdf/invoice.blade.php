    ...
    <div class="wrap">
        <div style="display:flex; justify-content:space-between; align-items:flex-start;">
            ...
            <div class="right">
                ...
                <div style="margin-top:8px;">
                    <span
                        class="badge {{ $p?->status === 'paid' ? 'paid' : '' }}">{{ strtoupper($p->status ?? 'pending') }}</span>
                </div>
            </div>
        </div>
        ...
        <div style="display:flex; gap:14px; margin-top:14px;">
            ...
            <div class="box" style="flex:1;">
                ...
                <div><strong>{{ Carbon::parse($tt->date)->translatedFormat('d F Y') }},
                        {{ \Carbon\Carbon::parse($tt->start_time)->format('H:i') }}
                        –
                        {{ \Carbon\Carbon::parse($tt->end_time)->format('H:i') }}
                    </strong></div>
            </div>
            ...
            <tbody>
                <tr>
                    <td>Coaching Session — Coach {{ $tt->coach->name }} ({{ ucfirst($tt->level) }})
                        <div class="muted">Tanggal {{ Carbon::parse($tt->date)->translatedFormat('d F Y') }},
                           {{ \Carbon\Carbon::parse($tt->start_time)->format('H:i') }}
                        –
                        {{ \Carbon\Carbon::parse($tt->end_time)->format('H:i') }}</div>
                    </td>
