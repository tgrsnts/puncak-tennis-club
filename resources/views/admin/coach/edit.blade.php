...
@section('content')
    <section id="dashboard" class="min-h-screen font-poppins w-full flex flex-col gap-4 p-4 pb-20 bg-[#F4F5F9]">
        <h2 class="text-2xl font-semibold mb-4">Edit Coach</h2>
        ...
        <div class="bg-white p-4 rounded-lg border border-gray-200 flex flex-col gap-4">
            <form action="{{ route('admin.coach.update', ['locale' => app()->getLocale(), 'id' => $coach->id]) }}"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                    ...
                    <div class="flex flex-col">
                        ...
                        <select name="specialty" id="specialty" class="w-full p-2 border border-slate-400 rounded-lg">
                            @php
                                $currentSpecialty = old('specialty', $coach->specialty);
                            @endphp
                            <option value="Beginner" {{ $currentSpecialty === 'Beginner' ? 'selected' : '' }}>Beginner
                            </option>
                            <option value="Intermediate" {{ $currentSpecialty === 'Intermediate' ? 'selected' : '' }}>
                                Intermediate</option>
                            <option value="Advanced" {{ $currentSpecialty === 'Advanced' ? 'selected' : '' }}>Advanced
                            </option>
                        </select>
                        @error('specialty')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div class="flex flex-col">
                        ...
                        <label for="photo_url"
                            ...
                            <img id="photo-preview"
                                class="{{ $coach->photo_url ? 'block' : 'hidden' }} w-full h-full object-cover"
                                src="{{ $coach->photo_url ? asset('storage/' . $coach->photo_url) : '' }}"
                                alt="Coach Photo" />
