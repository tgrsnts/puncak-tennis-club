...
@section('content')
    <section id="dashboard" class="min-h-screen font-poppins w-full flex flex-col gap-4 p-4 pb-20 bg-[#F4F5F9]">
        <h2 class="text-2xl font-semibold mb-4">Add Gallery Video</h2>
        ...
        <div class="bg-white p-4 rounded-lg border border-gray-200 flex flex-col gap-4">
            <form action="{{ route('admin.gallery-video.store', app()->getLocale()) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-6 items-start">
                    ...
                    <div id="desc-cell" class="flex flex-col">
                        ...
                        <textarea 
                            ...
                            name="content" id="content" rows="8">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <label for="video" id="video-box"
                            ...
                            <span id="video-name" class="hidden text-sm text-gray-600 ml-2"></span>
                        </label>

                        <input type="file" name="video" id="video" accept="video/*" hidden required>
                        <p class="text-sm text-gray-500 mt-1">Format: MP4, MOV, AVI, MKV, WebM (Max: 200MB)</p>
                        @error('video')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            videoInput.addEventListener('change', (e) => {
                ...
                if (file) {
                    ...
                    videoName.classList.add('hidden');
                }
            });
