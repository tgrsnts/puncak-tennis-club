...
@section('content')
<div class="flex flex-col gap-4 p-8 bg-white rounded-3xl shadow-xl w-full max-w-md">
  ...
  <form method="POST" action="/{{ app()->getLocale() }}/register/complete">
    ...
    <div>
      ...
      <input type="text" name="name" class="w-full px-4 py-2 border rounded-md bg-gray-100" required value="{{ old('name') }}">
      @error('name')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
    </div>

    <div>
      <label>Telepon</label>
      <input type="text" name="telepon" class="w-full px-4 py-2 border rounded-md bg-gray-100" required value="{{ old('telepon') }}">
